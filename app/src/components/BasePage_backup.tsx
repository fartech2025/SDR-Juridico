...
export default function BasePage({ children, maxWidth = "max-w-md", className = "" }: BasePageProps) {
  ...
  React.useEffect(() => {
    ...
    const loadLogo = async () => {
      try {
        // Primeiro, verificar se o arquivo existe tentando URL pública
        const { data: publicData } = supabase.storage.from(logoBucket).getPublicUrl(logoPath);
        ...
        if (publicData?.publicUrl) {
          try {
            ...
            console.warn('[BasePage] Logo público não acessível:', fetchError);
          }
        }
        
        // Se URL pública falhar, tentar URL assinada
        const { data: signedData, error: signedError } = await supabase.storage
          .from(logoBucket)
          .createSignedUrl(logoPath, 3600);
          
        if (signedError) {
          console.warn('[BasePage] Erro ao criar URL assinada:', signedError);
        } else if (signedData?.signedUrl && mounted) {
          setLogoUrl(signedData.signedUrl);
          return;
        }
        
      } catch (error) {
        console.error('[BasePage] Erro ao carregar logo:', error);
      }
      
      // Fallback para logo local se tudo falhar
      if (mounted) {
