...
export default function Home() {
  ...
  const loadAproveitamentoPorTema = async () => {
    try {
      ...
      if (temaData && temaData.length > 0) {
        const chartData = temaData.map((item: any) => ({
          nome: item.nome_tema?.substring(0, 10) || 'Tema',
          percentual: Math.round(item.percentual_acertos || 0)
        }));
        setAproveitamentoPorTema(chartData);
      } else {
        // Dados demo se não houver dados reais
        setAproveitamentoPorTema([
          { nome: 'Literatura', percentual: 85 },
          { nome: 'Gramática', percentual: 72 },
          { nome: 'Interpret.', percentual: 68 },
          { nome: 'Redação', percentual: 45 }
        ]);
      }
    } catch (error) {
      console.error('Erro ao carregar aproveitamento por tema:', error);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Carregar dados de aproveitamento por tema
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex">
      ...
      {/* Main Content */}
      <div className={`flex-1 p-3 sm:p-6 transition-all duration-300 ${sidebarCollapsed ? 'ml-0' : 'ml-0'}`}>
        <div className="max-w-6xl mx-auto space-y-6">{/* Header Moderno */}
