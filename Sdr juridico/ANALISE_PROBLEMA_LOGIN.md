...
### **Problema 1: RLS em USUARIOS bloqueia consulta do PermissionsService**
...
2. ❌ **MAS** as funções `is_fartech_admin()`, `get_user_org_id()`, `is_org_admin()` fazem SELECT em `USUARIOS`!
3. 🔄 **RECURSÃO OCULTA:** As policies chamam funções que consultam USUARIOS, criando dependência circular
...
3. `permissionsService.getCurrentUser()` consulta `USUARIOS`
4. RLS avalia policies:
   - `users_own_profile`: OK (auth.uid() existe)
   - **MAS** `fartech_admin_all_USUARIOS` chama `is_fartech_admin()`
   - `is_fartech_admin()` tenta SELECT em `USUARIOS`
...
### **Solução 2: Desabilitar RLS em USUARIOS.is_fartech_admin (MELHOR) ✅✅**
...
CREATE POLICY "fartech_all_USUARIOS" ON USUARIOS
  FOR ALL
  USING (
    (SELECT is_fartech_admin FROM USUARIOS WHERE user_id = auth.uid() LIMIT 1) = true
  );
```

---

### **Solução 3: Usar auth.jwt() ao invés de consultar USUARIOS ✅✅✅**
...
BEGIN
  ...
  NEW.raw_app_meta_data = jsonb_set(
    ...
    '{is_fartech_admin}',
    to_jsonb(COALESCE(
      (SELECT is_fartech_admin FROM USUARIOS WHERE user_id = NEW.id),
      false
    ))
  );
  RETURN NEW;
...
### **Solução 4: Remover RLS de USUARIOS (TEMPORÁRIA) ⚠️**
...
CREATE POLICY "fartech_admin_view_all" ON USUARIOS
  FOR SELECT
  USING (
...
CREATE POLICY "org_members_view" ON USUARIOS
  FOR SELECT
  USING (
    org_id = (
      SELECT p.org_id 
...
-- Policy 4: DELETE - Ninguém pode deletar USUARIOS
-- (sem policy = sem permissão)

-- NOTA: Para Fartech Admin ver todos os USUARIOS,
...
### Trade-offs:
- ⚠️ Fartech Admin **não** verá todos os USUARIOS via RLS
