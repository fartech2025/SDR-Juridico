...
DROP POLICY IF EXISTS "lead_scoring_configs_select_org_member" ON public.lead_scoring_configs;
CREATE POLICY "lead_scoring_configs_select_org_member"
  ON public.lead_scoring_configs
  FOR SELECT USING (is_org_member(org_id));

DROP POLICY IF EXISTS "lead_scoring_configs_insert_org_admin" ON public.lead_scoring_configs;
CREATE POLICY "lead_scoring_configs_insert_org_admin"
  ON public.lead_scoring_configs
  FOR INSERT WITH CHECK (is_org_admin_for_org(org_id));

DROP POLICY IF EXISTS "lead_scoring_configs_update_org_admin" ON public.lead_scoring_configs;
CREATE POLICY "lead_scoring_configs_update_org_admin"
  ON public.lead_scoring_configs
  FOR UPDATE USING (is_org_admin_for_org(org_id));

DROP POLICY IF EXISTS "lead_scoring_configs_delete_org_admin" ON public.lead_scoring_configs;
