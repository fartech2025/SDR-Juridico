...
export const CasoPage = () => {
    ...
    const { tarefas, loading: tarefasLoading, error: tarefasError, fetchTarefasByEntidade, } = useTarefas();
    ...
    const caseTasks = React.useMemo(() => tarefas.filter((task) => task.casoId === caso?.id), [tarefas, caso?.id]);
    ...
    const baseState = casosLoading || leadsLoading || docsLoading || agendaLoading || notasLoading || tarefasLoading
        ? 'loading'
    ...
    const checklistItems = React.useMemo(() => caseTasks.map((task) => ({
        id: task.id,
        label: task.title,
