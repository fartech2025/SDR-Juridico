...
import { getPermissionsByRole, FARTECH_ADMIN_PERMISSIONS, } from '@/types/permissions';
const resolveRoleFromPermissoes = (permissoes) => {
...
export const permissionsService = {
    ...
    async getCurrentUser() {
        try {
            ...
            const { data, error } = await supabase
                ...
                .single();
            if (error) {
                const fallbackName = (user.user_metadata && user.user_metadata.nome_completo) ||
                    user.email ||
                    'Usuario';
                return {
                    id: user.id,
    ...
    async checkPermission(check) {
        try {
            ...
            const hasPermission = permissions.some((p) => p.resource === check.resource &&
    ...
    async logPermissionCheck(check, result, metadata) {
        try {
            ...
            if (!sensitiveResources.includes(check.resource))
                return;
    ...
    async requirePermission(check) {
        ...
        if (!result.allowed) {
            throw new AppError(result.reason || 'Permissão negada', 'permission_denied');
        }
    },
    /**
    ...
    async requireFartechAdmin() {
        ...
        if (!isFartech) {
            throw new AppError('Esta operação requer permissões de administrador Fartech', 'permission_denied');
        }
    },
    /**
    ...
    async requireOrgAdmin() {
        ...
        if (!isAdmin) {
            throw new AppError('Esta operação requer permissões de administrador da organização', 'permission_denied');
        }
    },
};
