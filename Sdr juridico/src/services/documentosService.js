...
const buildDocumentoPayload = (doc, applyDefaults) => {
    const payload = {};
    if (doc.titulo !== undefined)
        payload.title = doc.titulo;
    if (doc.descricao !== undefined)
        payload.description = doc.descricao;
    if (doc.caso_id !== undefined)
        payload.caso_id = doc.caso_id;
    if (doc.mime_type !== undefined)
        payload.mime_type = doc.mime_type;
    if (doc.arquivo_tamanho !== undefined)
        payload.size_bytes = doc.arquivo_tamanho;
    if (doc.tags !== undefined)
        payload.tags = doc.tags;
    if (doc.url !== undefined)
        payload.storage_path = doc.url;
    const meta = {};
    if (doc.tipo !== undefined)
        meta.tipo = doc.tipo;
    if (doc.status !== undefined)
        meta.status = doc.status;
    if (doc.arquivo_nome !== undefined)
        meta.arquivo_nome = doc.arquivo_nome;
    if (doc.cliente_nome !== undefined)
        meta.cliente_nome = doc.cliente_nome;
...
export const documentosService = {
    ...
    async getDocumento(id) {
        try {
            ...
            if (error)
                throw new AppError(error.message, 'database_error');
            if (!data)
                throw new AppError('Documento nao encontrado', 'not_found');
    ...
    async createDocumento(documento) {
        try {
            ...
            if (error)
                throw new AppError(error.message, 'database_error');
            if (!data)
                throw new AppError('Erro ao criar documento', 'database_error');
            return mapDbDocumentoToDocumentoRow(data);
        }
        catch (error) {
    ...
    async obterUrlDocumento(storagePath) {
        try {
            ...
            if (error)
                throw new AppError(error.message, 'storage_error');
            if (!data.signedUrl)
                throw new AppError('Erro ao gerar URL', 'storage_error');
 ...
 */
export function obterIconeArquivo(tipoArquivo) {
    ...
    if (tipoArquivo.includes('word') || tipoArquivo.includes('document'))
        return '[DOC]';
    if (tipoArquivo.includes('excel') || tipoArquivo.includes('sheet'))
        return '[XLS]';
    return '[FILE]';
}
