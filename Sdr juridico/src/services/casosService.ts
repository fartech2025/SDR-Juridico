...
const prioridadeToNumber = (prioridade?: CasoRow['prioridade']) => {
...
const numberToPrioridade = (value?: number | null): CasoRow['prioridade'] => {
...
const mapDbStatusToUiStatus = (status?: string | null): CasoRow['status'] => {
...
const mapDbStatusToStage = (status?: string | null): CasoRow['stage'] | null => {
...
const normalizeStage = (value?: string | null): CasoRow['stage'] | null => {
...
const mapUiStageToDbStage = (stage?: CasoRow['stage'] | null): string | null => {
...
const mapUiStatusToDbStatus = (status?: CasoRow['status'] | null): string => {
  if (status === 'encerrado') return 'encerrado'
...
const mapUiStatusToDbFilter = (status: CasoRow['status']) => {
...
const buildCasoPayload = (caso: Partial<CasoRow>, applyDefaults: boolean) => {
  ...
  if (caso.titulo !== undefined) payload.titulo = caso.titulo
  if (caso.descricao !== undefined) payload.descricao = caso.descricao
  if (caso.area !== undefined) payload.area = caso.area
  if (caso.status !== undefined) payload.status = mapUiStatusToDbStatus(caso.status)
  if (caso.stage !== undefined) payload.fase_atual = mapUiStageToDbStage(caso.stage)
  if (caso.cliente_id !== undefined) payload.cliente_id = caso.cliente_id
  if (caso.lead_id !== undefined) payload.lead_id = caso.lead_id
  if (caso.stage !== undefined) payload.fase_atual = caso.stage
  if (caso.valor !== undefined) payload.valor_estimado = caso.valor
  if (caso.data_encerramento !== undefined) payload.encerrado_em = caso.data_encerramento
...
export const casosService = {
  ...
  async getCasosByCliente(clienteId: string): Promise<CasoRow[]> {
    try {
      ...
      const { data, error } = isFartechAdmin
        ...
        : await query.eq('org_id', orgId).order('created_at', { ascending: false })
  ...
  async getEstatisticas(): Promise<{
    ...
    try {
      ...
      return {
        total: casos.length,
        abertos: casos.filter((c: any) => c.status === 'triagem').length,
        andamento: casos.filter((c: any) => ['andamento', 'negociacao'].includes(c.status)).length,
        resolvidos: casos.filter((c: any) => c.status === 'contrato').length,
        fechados: casos.filter((c: any) => c.status === 'arquivado').length,
        encerrados: casos.filter((c: any) => c.status === 'encerrado').length,
        criticos: casos.filter((c: any) => [3, 4].includes(c.prioridade)).length,
      }
    } catch (error) {
