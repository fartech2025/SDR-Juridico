...
export function useFartechAdmin() {
  ...
  const getOrgsWithAlerts = useCallback(async () => {
    try {
      ...
      const orgsData = allOrgs || []
      const alertOrgs: Array<Organization & { alerts: string[] }> = []
   ...
   */
  const viewOrganization = useCallback(async (orgId: string) => {
    try {
      setLoading(true)
      setError(null)
 ...
 */
export function useIsViewingOrg(): boolean {
 ...
 */
export function useIsFartechDashboardView(): boolean {
  const { currentOrg } = useOrganization()
  const isFartechAdmin = useIsFartechAdmin()
