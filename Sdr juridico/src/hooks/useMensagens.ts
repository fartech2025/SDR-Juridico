...
interface UseMensagensState {
  mensagens: LeadMessage[]
  loading: boolean
  error: Error | null
}

const mapToLeadMessage = (row: { id: string; direction: string; body: string | null; created_at: string }) => ({
  id: row.id,
...
export function useMensagens(leadId?: string) {
  const [state, setState] = useState<UseMensagensState>({
    mensagens: [],
    loading: false,
    error: null,
  })
