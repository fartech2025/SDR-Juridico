...
export const stripChecklistPrefix = (title: string) => {
...
export const hasChecklistPrefix = (title: string) =>
  title.trim().startsWith(CHECKLIST_PREFIX)

export const ensureChecklistPrefix = (title: string) => {
  const trimmed = title.trim()
  if (!trimmed) return ''
